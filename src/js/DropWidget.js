export default class DropWidget {
  ...
  onInputFile() {
    ...
    this.onAdd(URL.createObjectURL(this.inputEl.files[0]));
  }

  onAdd(src) {
    this.addEventListeners.forEach((c) => c.call(null, src));
  }
  ...
  onDrop(event) {
    ...
    files.forEach((f) => {
      if (f.type.startsWith('image/')) this.onAdd(URL.createObjectURL(f));
    });
    this.dropAreaEl.classList.remove('dragover');
  }
}
